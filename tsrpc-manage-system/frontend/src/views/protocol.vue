    ...
    <div class="protocol-page">
        <div class="container">
            <el-tabs v-model="activeTab" type="border-card">
                <!-- 协议列表：增删改查，点击编辑可编辑请求/响应字段，字段=类型+名字，类型可选基础/自定义 -->
                <el-tab-pane label="协议列表" name="services">
                    <div class="tab-header">
                        <span>长连接协议 · 协议 ID、名称 · 支持增删改查，点击编辑可修改请求/响应字段</span>
                        <div>
                ...
                <!-- 自定义类型：可增删改，字段支持基础类型+自定义类型嵌套 -->
                <el-tab-pane label="自定义类型" name="customTypes">
                    <div class="tab-header">
                        <span>模块内复用或供协议字段选择的类型 · 字段支持基础类型与自定义类型嵌套</span>
                ...
                <el-tab-pane label="导出 d.ts" name="export">
                    <div class="export-section">
                        <div class="export-desc">将当前协议与自定义类型生成 TypeScript 声明文件并下载，可自定义导出文件名。</div>
                        <el-form label-width="120px" class="export-form">
                            <el-form-item label="导出路径/文件名">
                                <el-input
                                    v-model="exportFilename"
                                    placeholder="例如: protocol.d.ts"
                                    clearable
                                />
                            </el-form-item>
                            <el-form-item>
...
const servicesDisplayed = computed(() =>
    services.value.filter((s) => (s.type || 'api') === 'msg')
);
...
function saveProtocolEdit() {
    ...
    types.value = t;
    // 同步到列表中的 service 项（含可选的 reqTypeId/resTypeId）
...
/** 从协议名生成请求/响应类型 id（格式：name/ReqX、name/ResX，中间不加 Ptl） */
...
function confirmAddProtocol() {
    ...
    services.value = [...services.value, { id, name, type: 'msg', reqTypeId, resTypeId }];
    types.value = {
        ...types.value,
